...
class ToolRegistry @Inject constructor(
    ...
    private fun nodeLabelForId(service: DrishtiAccessibilityService, id: Int): String? {
     ...
     */
    private fun findBestSearchNode(service: DrishtiAccessibilityService): UiNode? {
        val screenH = service.resources.displayMetrics.heightPixels.coerceAtLeast(1)
        val topBandPx = screenH * ADDRESS_TOP_BAND_FRAC
    ...
    private fun shouldApplyFoodSearchAddressGuard(service: DrishtiAccessibilityService): Boolean {
    ...
    private fun resolveAnnotateBounds(nodeId: Int?): Rect? {
        ...
        val service = DrishtiAccessibilityService.instance ?: return null
    ...
    private fun a11yOrNull(): DrishtiAccessibilityService? {
        val s = DrishtiAccessibilityService.instance
        if (s == null) DrishtiAccessibilityService.showEnableToast()
        return s
    }
    ...
    private fun currentForegroundPackage(service: DrishtiAccessibilityService): String {
    ...
    private fun boundsOnScreen(service: DrishtiAccessibilityService, bounds: Rect): Boolean {
