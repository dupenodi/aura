...
import com.drishti.voice.VoiceCoordinator
...
class OverlayService : Service() {
                ...
                if (!ensureWindowsAttached()) {
                    toast(
                        if (!Settings.canDrawOverlays(this@OverlayService)) {
                            "Grant overlay permission for Drishti (com.drishti) first"
                        } else {
    ...
    /** Nuclear recovery: strip every Drishti overlay we manage, then add exactly one pair. */
    ...
    private fun showBubbleMenu(anchor: BubbleView) {
        runCatching {
            ...
            popup.menu.add(0, 4, 2, "Open Drishti home")
            ...
            popup.menu.add(0, 3, 5, "Stop Drishti overlay")
    ...
    companion object {
        const val ACTION_START = "com.drishti.overlay.START"
        const val ACTION_STOP = "com.drishti.overlay.STOP"
        const val ACTION_DEBUG_DEMO = "com.drishti.overlay.DEBUG_DEMO"
        const val ACTION_REATTACH = "com.drishti.overlay.REATTACH"

        private const val CHANNEL_ID = "drishti_overlay"
        private const val NOTIFICATION_ID = 42
        private const val REATTACH_DEBOUNCE_MS = 280L
        private const val WATCHDOG_MS = 2_000L
        private const val WINDOW_TITLE_CANVAS = "DrishtiAnnotations"
        private const val WINDOW_TITLE_BUBBLE = "DrishtiBubble"
