...
import com.clicky.ai.LlmProvider
import com.clicky.ai.LlmRouter
import com.clicky.ai.MessageContent
import com.clicky.ai.OpenAiClient
import com.clicky.ai.Speaker
import com.clicky.ai.ToolCall
...
class AgentLoop @Inject constructor(
    ...
    private suspend fun handleSaveFlowRequest(utterance: String): Result<String> {
        ...
        val speechLang = state.lastDetectedLanguage
            ?: preferenceStore.get("language")?.let {
                com.clicky.ai.SarvamClient.normalizeTtsLanguage(it)
            }
            ?: "en-IN"
    ...
    private suspend fun runLocked(utterance: String): Result<String> {
        ...
        if (ClickyAccessibilityService.instance == null) {
            ...
            val msg = "Enable Clicky in Accessibility settings"
            state.updateStatus(AgentRunStatus.Error, msg, lastError = msg)
            toast(msg)
            return Result.failure(IllegalStateException(msg))
        }
        ...
        var model = if (preferAnthropic) {
            com.clicky.ai.AnthropicClient.MODEL
        } else {
            ...
            while (iteration < MAX_ITERATIONS && finishedSummary == null) {
                ...
                if (toolCalls.isEmpty()) {
                    val content = when (val c = assistantMsg.content) {
                        is com.clicky.ai.MessageContent.Text -> c.text
    ...
    private fun screenSize(): Pair<Int, Int> {
        val service = ClickyAccessibilityService.instance
        if (service != null) {
        ...
        val ctx = ClickyApp.appContextOrNull()
